...
if (SERVER) then
	...
	function SWEP:DrawHUD()
		local w, h = ScrW(), ScrH()
		local cury = h/4*3
		local tx, ty = draw.SimpleText("Left Click: Select Entity", "ixMenuButtonFont", w/2, cury, color_white, 1, 1)
		cury = cury + ty
		local tx, ty = draw.SimpleText("Right Click: Deselect Entity", "ixMenuButtonFont", w/2, cury, color_white, 1, 1)
		cury = cury + ty
		local tx, ty = draw.SimpleText("Reload: Register Area", "ixMenuButtonFont", w/2, cury, color_white, 1, 1)
	end
	...
	hook.Add("PostDrawOpaqueRenderables", "helperDrawModel", function()
		...
		if (ent and IsValid(ent)) then
			if (HELPER_INFO.modelObject and IsValid(HELPER_INFO.modelObject)) then
				local dPos, dAng = ent:GetBonePosition(ent:LookupBone("ValveBiped.Bip01_R_Hand"))
